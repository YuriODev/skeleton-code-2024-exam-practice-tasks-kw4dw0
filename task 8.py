...
class Puzzle():
        ...
        while not Finished:
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                if AmountToAddToScore > 0:
                    ...
                    reshuffle = input("Do you want to shuffle blocked cells? Press enter to ignore: ")
                    if len(reshuffle) > 0:
    ...
    def ReShuffleBlockedCells(self):
        Blocked = [Cell for Cell in self.__Grid if Cell.GetSymbol() == "@"]
        for item in Blocked:
            oldPos = self.__Grid.index(item)
            newPos = oldPos
            while self.__Grid[newPos].GetSymbol() != "-" and newPos == oldPos:
                newPos = random.randrange(1, self.__GridSize * self.__GridSize + 1)
            self.__Grid[oldPos] = self.__Grid[newPos]
            self.__Grid[newPos] = item
        print("Shuffle coplete!")
