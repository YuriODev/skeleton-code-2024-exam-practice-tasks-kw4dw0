...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
    def GetSpaceLeftOnGrid(self):
        PatternsLeft = 0
        for StartRow in range(self.__GridSize):
            for StartColumn in range(self.__GridSize):
                try:
                    PatternString = ""
                    PatternString += self.__GetCell(StartRow, StartColumn).GetSymbol()
                    PatternString += self.__GetCell(StartRow, StartColumn + 1).GetSymbol()
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        if CurrentSymbol == P.GetPatternSequence()[0] and not P.MatchesPattern(PatternString, CurrentSymbol):
                            symbolsNeeded = 0
                            for i in range(len(PatternString)):
                                if PatternString[i] != P.GetPatternSequence()[i] and PatternString[i] != "@":
                                    symbolsNeeded += 1
                            if symbolsNeeded <= self.__SymbolsLeft:
                                pass
                            else:
